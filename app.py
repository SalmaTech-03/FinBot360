...
with st.sidebar:
        ...
        if user_text:
            with st.spinner("Analyzing..."):
                try:
                    ...
                    st.write(f"**Sentiment:** {results[0]['label'].title()}")
                    st.write(f"**Confidence:** {results[0]['score']:.2f}")
                except Exception as e:
    ...
    with st.expander("📁 Portfolio Performance Analysis", expanded=True):
        ...
        if uploaded_file is not None:
            ...
            st.success("File uploaded. Analysis would be shown here.")
