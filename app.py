...
def forecast_stock(data: pd.DataFrame):
    ...
    # 6. --- THE DEFINITIVE FIX: Manually build the final dataframes ---
    train_df = data_close.iloc[:training_data_len]
    
    # Get the specific dates for the validation period from the original dataframe
    ...
    # This handles any possible length mismatches between predictions and available dates
...
with st.sidebar:
    ...
    with st.expander("📁 Portfolio Performance Analysis"):
        uploaded_file = st.file_uploader("Upload Portfolio", type=['csv', 'xlsx'], key="portfolio_uploader")
    ...
    with st.expander("📊 Stock Forecasting", expanded=True):
        ...
        if st.button("Generate Forecast", key="forecast_button"):
            # Fetch data from 2000 to get a long history for stocks like NVDA
            data = fetch_stock_data(ticker, "2000-01-01", pd.to_datetime("today").strftime('%Y-%m-%d'))
            if not data.empty:
                train, valid = forecast_stock(data)
                if train is not None:
                    # Use session state to hold the figure
        ...
        # Display the figure from session state if it exists
