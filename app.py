...
def get_news_and_sentiment(ticker: str):
    ...
    elif avg_score < -0.3: overall_sentiment, color = "Negative", "red"
    else: overall_sentiment, color = "Neutral", "orange"
    return overall_sentiment, color, headlines

# --- CORRECTED `fetch_stock_data` FUNCTION ---
@st.cache_data
def fetch_stock_data(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    # --- THE DEFINITIVE FIX IS HERE: `date` has been corrected to `end_date` ---
    ...
    with st.expander("🔴 Live Market Dashboard", expanded=True):
        ...
        if ticker_symbol:
            with st.spinner(f"Fetching news for {ticker_symbol}..."):
                ...
                with col1: st.markdown(f"**Overall Sentiment:**")
                with col2: st.markdown(f"**<p style='color:{color};'>{sentiment}</p>**", unsafe_allow_html=True)
